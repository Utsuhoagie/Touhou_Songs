...
class RegisterHandler : BaseHandler<RegisterCommand, RegisterResponse, Result<RegisterResponse>>
{
	...
	public override async Task<Result<RegisterResponse>> Handle(RegisterCommand command, CancellationToken cancellationToken)
	{
		...
		return Ok(new RegisterResponse(newUser.UserName!, newUser.Email!));
	}
}
